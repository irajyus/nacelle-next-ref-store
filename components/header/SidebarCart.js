...
const SidebarCart = () => {
  ...
  useEffect(() => {
    nacelleClient.spaceProperties().then(({ properties }) => {
      ...
      nacelleClient.products({ handles: upsellHandles }).then((products) => {
        setUpsells(products.map((prod) => ({ ...prod, hasAdded: false })));
      });
    });
  }, []);
  const handleAddItem = (upsell) => {
    ...
    setUpsells((prev) =>
      prev.map((u) => ({
        ...u,
        hasAdded: u.sourceEntryId === upsell.sourceEntryId ? true : u.hasAdded
      }))
    );
  };
  return (
              ...
              <div className="w-screen max-w-md transform transition ease-in-out duration-500">
                <div className="h-full flex flex-col bg-white shadow-xl overflow-y-scroll">
                  ...
                  <div className="border-t border-gray-200 py-4 px-4">
                    ...
                    <ul role="list" className="-mb-6 divide-y divide-gray-200">
                      {upsells.map((upsell, i) => (
                        <li className="py-6 flex" key={i}>
                          <div className="relative flex-shrink-0 border border-gray-200 rounded-md overflow-hidden w-20 h-20">
                            <Image
                              src={upsell.variants[0].content.featuredMedia.src}
                              alt={upsell.variants[0].content.productTitle}
                              layout="fill"
                              objectFit="cover"
                            />
                          </div>
